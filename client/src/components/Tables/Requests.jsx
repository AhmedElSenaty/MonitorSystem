      ...
      try {
        const res = await api.get(`/api/Request`, {
          params: {
            ...
            degreeType: degreeFilter === "" ? null : degreeFilter,
            employeeType: employeeTypeFilter === "" ? null : employeeTypeFilter,
          },
        });
        ...
        if (
          ...
          (user.role !== null || user.role.toLowerCase() !== "employee")
        ) {
          ...
          <div className="summary mb-4">
            <div className="row justify-content-end">
              {["تحت المراجعة", "تم القبول", "تم الرفض"].map((status) => (
                ...
                >
                  <div
                    className={`card status-card text-end border-${
                      status === "تم القبول"
                        ? "success"
                        : status === "تم الرفض"
                        ? "danger"
                ...
                {requests.length > 0 ? (
                  requests.map((request, index) => (
                    <tr key={index}>
                      ...
                      <td className="text-break">{request.employeeType}</td>
                      ...
                      <td className="text-center">
                        <button
                          ...
                          onClick={() =>
                            navigate(`/profile/${request.ssn}/${request.id}`)
                          }
                        >
                          عرض
                        </button>
